...
class PatternCategory extends Component {
    componentDidMount() {
    ...
    render() {
        ...
        const { pizza, categotyId} = this.props;
        var filetrCategory = pizza.filter(p => p.categoryId === categotyId)
        return (
            <div style={styles.root}>
                <Paper style={style} zDepth={1} >
                    <GridList
                        ...
                        cols={cols}
                    >
                        {filetrCategory.map((p, index) =>  this.renderPizza(p, index))}
                    </GridList>
                </Paper>
            </div>
        )
...
export default connect(mapStateToProps, mapDispatchToProps)(PatternCategory)
